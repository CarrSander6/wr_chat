...
import com.bx.implatform.dto.IsRobDTO;
import com.bx.implatform.dto.RedPacketContentDTO;
...
//        Date minDate = DateUtils.addMonths(new Date(), -months);
        // 只拉取24小时内的消息
        Date minDate = DateUtils.addHours(new Date(), -24);
        ...
        EXECUTOR.execute(() -> {
            ...
            messageGroupMap.forEach((groupId, groupMessages) -> {
                ...
                for (GroupMessage m : sendMessages) {
                    ...
                    // todo:如果是红包消息，则要额外处理
                    /*if (m.getType().equals(MessageType.RED_PACKAGE.code())) {
                        RedPacketContentDTO redPacketContentDTO = JSONUtil.toBean(m.getContent(), RedPacketContentDTO.class);
                        IsRobDTO isRobDTO = new IsRobDTO();
                        isRobDTO.setUserId(session.getUserId());
                        isRobDTO.setRedId(redPacketContentDTO.getId());
                        Boolean rob = userService.isRob(isRobDTO);
                        redPacketContentDTO.setRob(rob);
                        m.setContent(JSONUtil.toJsonStr(redPacketContentDTO));
                    }*/
