...
public class FriendServiceImpl extends ServiceImpl<FriendMapper, Friend> implements FriendService {
    ...
    @Override
    public void addFriendAuto(FriendRequest request) {
        if (request.getSendId().equals(request.getRecvId())) {
            throw new GlobalException("不允许添加自己为好友");
        }
        // 互相绑定好友关系
        FriendServiceImpl proxy = (FriendServiceImpl)AopContext.currentProxy();
        proxy.bindFriend(request.getSendId(), request.getRecvId());
        proxy.bindFriend(request.getRecvId(), request.getSendId());
        // 推送添加好友提示
        sendAddTipMessageAuto(request);
        log.info("添加好友，用户id:{},好友id:{}", request.getSendId(), request.getRecvId());
    }
     ...
     * @param request 好友id
     */
    void sendAddTipMessageAuto(FriendRequest request) {
        //UserSession session = SessionContext.getSession();
        PrivateMessage msg = new PrivateMessage();
        msg.setSendId(request.getSendId());
        msg.setRecvId(request.getRecvId());
        msg.setContent("你们已成为好友，现在可以开始聊天了");
        msg.setSendTime(new Date());
        msg.setStatus(MessageStatus.UNSEND.code());
        msg.setType(MessageType.TIP_TEXT.code());
        ...
        PrivateMessageVO messageInfo = BeanUtils.copyProperties(msg, PrivateMessageVO.class);
        IMPrivateMessage<PrivateMessageVO> sendMessage = new IMPrivateMessage<>();
        sendMessage.setSender(new IMUserInfo(request.getSendId(), null));
        sendMessage.setRecvId(request.getRecvId());
        sendMessage.setSendToSelf(false);
        sendMessage.setData(messageInfo);
        imClient.sendPrivateMessage(sendMessage);
        // 推给自己
        sendMessage.setRecvId(request.getSendId());
        imClient.sendPrivateMessage(sendMessage);

        // 发送greet消息
        //User user = userMapper.selectById(request.getRecvId());
        List<String> msgList = StrUtil.split(userMapper.selectConfigByKey("biz.im.msg"), ";");
        if (CollectionUtil.isNotEmpty(msgList)) {
            for (String greetMsg : msgList) {
                PrivateMessage messageInfoGreet = new PrivateMessage();
                messageInfoGreet.setSendId(request.getRecvId());
                messageInfoGreet.setRecvId(request.getSendId());
                messageInfoGreet.setContent(greetMsg);
                messageInfoGreet.setSendTime(new Date());
                messageInfoGreet.setType(MessageType.TEXT.code());
                messageInfoGreet.setStatus(MessageStatus.UNSEND.code());
                privateMessageMapper.insert(messageInfoGreet);

                PrivateMessageVO messageInfoGreetVo = BeanUtils.copyProperties(messageInfoGreet, PrivateMessageVO.class);

                IMPrivateMessage<PrivateMessageVO> sendGreetMessage = new IMPrivateMessage<>();
                sendGreetMessage.setSender(new IMUserInfo(request.getRecvId(), null));
                sendGreetMessage.setRecvId(request.getSendId());
                sendGreetMessage.setSendToSelf(false);
                sendGreetMessage.setData(messageInfoGreetVo);
                imClient.sendPrivateMessage(sendGreetMessage);
            }
        }
    }
