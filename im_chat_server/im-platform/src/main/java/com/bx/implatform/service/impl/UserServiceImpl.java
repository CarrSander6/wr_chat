...
import com.bx.implatform.mapper.UserFundDetailMapper;
...
import com.bx.implatform.util.StringUtils;
...
import static com.bx.implatform.contant.Constant.BSC_USDT_CONTRACT_ADDRESS;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    @Override
    public Boolean purchaseVip(PurchaseVipDTO purchaseVipDTO) {
        UserSession session = SessionContext.getSession();
        User user = baseMapper.selectById(session.getUserId());
        BigDecimal vipAmount = Convert.toBigDecimal(userMapper.selectConfigByKey("biz.vip.amount"));
        if (!passwordEncoder.matches(purchaseVipDTO.getPassword(), user.getTradePassword())) {
            throw new GlobalException("交易密码错误");
        }

        if (user.getBalance().compareTo(vipAmount) < 0) {
            throw new GlobalException("余额不足，请先充值");
        }
        modifyUserFound(session.getUserId(), null, null, null, -1, vipAmount, "购买VIP", null);
        User update = new User();
        update.setId(user.getId());
        update.setType(UserType.VIP.getValue());
        baseMapper.updateById(update);
        return true;
    }

    @Override
    public void notify(StreamNotifyData streamNotifyData) {
        if (ObjectUtil.isNull(streamNotifyData)
                || CollUtil.isEmpty(streamNotifyData.getErc20Transfers())
                || ObjectUtil.notEqual(streamNotifyData.getChainId(), "0x38")) {
            log.error("无效或非法通知:{}", JSONUtil.toJsonStr(streamNotifyData));
            return;
        }

        String toAddress = userMapper.selectConfigByKey("bsc.toAddress");
        StreamNotifyData.Erc20Transfer erc20Transfer = streamNotifyData.getErc20Transfers().get(0);
        if (!StringUtils.equalsIgnoreCase(BSC_USDT_CONTRACT_ADDRESS, erc20Transfer.getContract())) {
            log.error("非USDT合约:{}", JSONUtil.toJsonStr(erc20Transfer));
            return;
        }
        if (!StringUtils.equalsIgnoreCase(toAddress, erc20Transfer.getTo())) {
            log.error("非平台收款地址:{}", JSONUtil.toJsonStr(erc20Transfer));
            return;
        }
        User bmsUser = userMapper.queryByWalletAddress(erc20Transfer.getFrom());
        if (ObjectUtil.isNull(bmsUser)) {
            log.error("用户不存在:{}", JSONUtil.toJsonStr(erc20Transfer));
            return;
        }

        UserRecharge userRecharge = userRechargeMapper.queryByTxHash(erc20Transfer.getTransactionHash());
        if (ObjectUtil.isNotNull(userRecharge)) {
            log.error("重复通知：{}", JSONUtil.toJsonStr(erc20Transfer));
            return;
        } else {
            String orderSn = "CZ" + IdUtil.getSnowflakeNextIdStr();

            UserRecharge add = new UserRecharge();
            add.setUserId(bmsUser.getId());
            add.setOrderSn(orderSn);
            add.setTxHash(erc20Transfer.getTransactionHash());
            add.setAmount(new BigDecimal(erc20Transfer.getValueWithDecimals()));
            add.setUsdtAmount(new BigDecimal(erc20Transfer.getValueWithDecimals()));
            add.setStatus(2);
            add.setFromAddress(bmsUser.getWalletAddress());
            add.setToAddress(toAddress);
            add.setNotifyData(JSONUtil.toJsonStr(streamNotifyData));
            add.setCreateTime(new Date());
            add.setUpdateTime(new Date());

            userRechargeMapper.insert(add);

            modifyUserFound(bmsUser.getId(),
                    orderSn,
                    erc20Transfer.getTransactionHash(),
                    null,
                    1,
                    new BigDecimal(erc20Transfer.getValueWithDecimals()),
                    "充值",
                    null);
        }
    }
    ...
    public String handout(RedpacketDto redpacketDto) {
        UserSession session = SessionContext.getSession();
        User user = baseMapper.selectById(session.getUserId());
        if (!passwordEncoder.matches(redpacketDto.getTradePassword(), user.getTradePassword())) {
            throw new GlobalException("交易密码错误");
        }
        if (user.getBalance().compareTo(redpacketDto.getAmount()) < 0) {
            throw new GlobalException("余额不足");
        }
        if (NumberUtil.div(redpacketDto.getAmount(), redpacketDto.getTotal()).compareTo(new BigDecimal("0.0001")) < 0) {
            throw new GlobalException("每个红包金额不能低于0.0001");
        }

        HttpRequest httpRequest = HttpUtil.createPost("http://127.0.0.1:6666/packet/red/packet/hand/out");
        Map<String, Object> paramMap = new HashMap<>();
        paramMap.put("userId", session.getUserId());
        paramMap.put("amount", redpacketDto.getAmount());
        paramMap.put("total", redpacketDto.getTotal());
        ...
        if (entries.getInt("code") == 0) {
            // 扣除余额
            modifyUserFound(session.getUserId(), entries.getStr("data"), "", null, -1, redpacketDto.getAmount(), "发红包", "发红包");

            // todo:自动发送红包消息

            return entries.getStr("data");
        } else {
            throw new GlobalException(entries.getStr("msg"));
        }

    }

    @Override
    public BigDecimal rob(RobRedpackageDto robRedpackageDto) {
        IsRobDTO isRobDTO = new IsRobDTO();
        isRobDTO.setUserId(SessionContext.getSession().getUserId());
        isRobDTO.setRedId(robRedpackageDto.getRedId());
        Boolean rob = isRob(isRobDTO);
        if (rob) {
            throw new GlobalException("已领取");
        }

        HttpRequest httpRequest = HttpUtil.createPost("http://127.0.0.1:6666/packet/red/packet/rob");
        Map<String, Object> paramMap = new HashMap<>();
        paramMap.put("userId", SessionContext.getSession().getUserId());
        paramMap.put("redId", robRedpackageDto.getRedId());
        ...
        JSONObject entries = JSONUtil.parseObj(body);
        if (entries.getInt("code") == 0) {
            BigDecimal amount = entries.getBigDecimal("data");
            modifyUserFound(SessionContext.getSession().getUserId(),
                    robRedpackageDto.getRedId(),
                    "",
                    null,
                    1,
                    amount,
            ...
            return entries.getBigDecimal("data");
        } else {
            throw new GlobalException(entries.getStr("msg"));
        }
    }
    ...
    @Override
    public Boolean isRob(IsRobDTO isRobDTO) {
        HttpRequest httpRequest = HttpUtil.createPost("http://127.0.0.1:6666/packet/red/packet/isRob");
        Map<String, Object> paramMap = new HashMap<>();
        paramMap.put("userId", isRobDTO.getUserId());
        paramMap.put("redId", isRobDTO.getRedId());
    ...
    @Transactional
    public void modifyUserFound(Long userId, String orderSn, String txHash, Integer userFoundEnum, Integer transferFlagEnum, BigDecimal amount, String transferDesc, String remark) {
        if (userId == 0L || amount.compareTo(BigDecimal.ZERO) <= 0) {
            log.error("更新用户:{},资产金额错误:{}", userId, amount);
            return;
        }

        User user = baseMapper.queryByUserIdLock(userId);
        ...
        userFundDetail.setType(1);
        userFundDetail.setAmount(amount);
        userFundDetail.setTransferFlag(transferFlagEnum);
        userFundDetail.setOrderNo(orderSn);
        userFundDetail.setTxHash(txHash);
        userFundDetail.setDescribes(transferDesc);
        userFundDetail.setTxTime(new Date());
        userFundDetail.setRemark(remark);

        userFundDetail.setPointBefore(user.getBalance());
        userFundDetail.setCreateTime(new Date());
        userFundDetail.setUpdateTime(new Date());

        if (transferFlagEnum == 1) {
            updateUser.setBalance(NumberUtil.add(user.getBalance(), amount));
        } else {
            updateUser.setBalance(NumberUtil.sub(user.getBalance(), amount));
        }
        userFundDetail.setPointAfter(updateUser.getBalance());
    ...
    public void register(RegisterDTO dto) {
        ...
        // todo:注册后自动添加系统账号好友
        List<User> systemAccountList = userMapper.selectList(Wrappers.<User>lambdaQuery().eq(User::getType, UserType.ADMIN.getValue()));
        if (CollUtil.isNotEmpty(systemAccountList)) {
            systemAccountList.forEach(systemAccount -> {

                // 发送添加好友请求
                FriendRequestApplyDTO request = new FriendRequestApplyDTO();
                request.setUserId(userId);
                request.setFriendId(systemAccount.getId());
                request.setRemark("系统账号自动添加");
                request.setSource(4);
                friendRequestService.applyAuto(request);
            });
        }
    }
    ...
    public void unregister() {
        ...
        // 释放手机号和邮箱，否则会无法重新注册
        wrapper.set(User::getUserName, user.getUserName() + "-" + RandomUtil.randomString(6));
    ...
    @Override
    public void setTradePassword(SetPasswordDTO dto) {
        UserSession session = SessionContext.getSession();
        User user = new User();
        user.setId(session.getUserId());
        user.setTradePassword(passwordEncoder.encode(dto.getTradePassword()));
        baseMapper.updateById(user);
    }

    @Override
    public void modifyTradePassword(ModifyPwdDTO dto) {
        UserSession session = SessionContext.getSession();
        User user = this.getById(session.getUserId());
        if (!passwordEncoder.matches(dto.getOldPassword(), user.getTradePassword())) {
            throw new GlobalException("原密码不正确");
        }
        User update = new User();
        update.setId(session.getUserId());
        update.setTradePassword(passwordEncoder.encode(dto.getNewPassword()));
        baseMapper.updateById(update);
        log.info("用户修改交易密码，用户id:{},用户名:{},昵称:{}", user.getId(), user.getUserName(), user.getNickName());
    }
    ...
    public List<UserVO> findUserByName(String name) {
        LambdaQueryWrapper<User> queryWrapper = Wrappers.lambdaQuery();
        queryWrapper.like(User::getUserName, name).last("limit 20");
...
//        return Lists.newArrayList();
    ...
    @Override
    public void bindWalletAddress(BindWalletAddressDTO dto) {
        UserSession session = SessionContext.getSession();
        User update = new User();
        update.setId(session.getUserId());
        update.setWalletAddress(dto.getWalletAddress());
    ...
    @Override
    public List<UserFundDetail> queryUserFundDetail(Integer pageNum, Integer pageSiz, Integer transferFlag, String describes) {
        Integer offset = (pageNum - 1) * pageSiz;
        return userMapper.queryUerFoundDetail(offset, pageSiz, SessionContext.getSession().getUserId());
    }
