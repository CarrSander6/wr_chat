...
import com.bx.implatform.dto.SetPasswordDTO;
...
public class LoginController {
    ...
    @PutMapping("/setTradePwd")
    @Operation(summary = "设置密码", description = "设置密码")
    public Result setTradePwd(@Valid @RequestBody SetPasswordDTO dto) {
        userService.setTradePassword(dto);
        return ResultUtils.success();
    }

    @PutMapping("/modifyTradePassword")
    @Operation(summary = "修改交易密码", description = "修改交易密码")
    public Result modifyTradePassword(@Valid @RequestBody ModifyPwdDTO dto) {
        userService.modifyTradePassword(dto);
        return ResultUtils.success();
    }
