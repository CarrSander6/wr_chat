...
import com.bx.implatform.enums.UserType;
...
public class GroupServiceImpl extends ServiceImpl<GroupMapper, Group> implements GroupService {
    ...
    public GroupVO createGroup(GroupVO vo) {
        ...
        // 校验是否能够创建群聊
        ArrayList<Integer> vipList = CollUtil.newArrayList(UserType.VIP.getValue(), UserType.ADMIN.getValue());
        if (!vipList.contains(user.getType())) {
            throw new GlobalException("vip用户才能创建群聊");
        }
        Long count = baseMapper.selectCount(Wrappers.<Group>lambdaQuery().eq(Group::getOwnerId, user.getId()));
        Long createGroupNum = Convert.toLong(userMapper.selectConfigByKey("biz.create.group"));
        if (user.getType().equals(UserType.VIP.getValue()) && count + 1 > createGroupNum) {
            throw new GlobalException("vip用户最多创建" + createGroupNum + "个群聊");
        }
