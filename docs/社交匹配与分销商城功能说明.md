...
3. 选择数量和支付方式
4. 创建订单并完成支付
5. 如果商品支持分销，付款后生成待结算佣金；订单完成时结算入账；售后批准取消待结算佣金

## 注意事项

1. **分销合规性**：确保分销机制符合当地法律法规，仅支持二级分销
2. **支付安全**：iOS内购需要严格验证receipt，防止欺诈
3. **佣金结算**：订单完成后结算佣金；售后批准会取消待结算佣金并回滚库存
...
#### 1. 智能匹配推荐系统
已实现基于多维度的智能推荐算法:
- **综合评分系统**: 考虑性别偏好(30分)、年龄差距(20分)、兴趣相似度(20分)、活跃度(15分)、资料完整度(15分)
- **兴趣推荐**: 基于用户兴趣标签匹配相似用户
- **地理位置推荐**: 支持基于经纬度的附近的人功能(需要数据库支持空间索引)
- **匹配度计算**: 实时计算两个用户之间的匹配分数

**使用方式**:
```
GET /match/recommend/smart?limit=10          # 智能推荐
GET /match/recommend/by-interest?limit=10    # 兴趣推荐
GET /match/recommend/nearby?longitude=113.23&latitude=23.16&radius=10&limit=10  # 附近的人
GET /match/recommend/match-score?targetUserId=123  # 计算匹配度
```

#### 2. 分销统计报表系统
已实现完整的分销数据统计和分析功能:
- **数据概览**: 今日/昨日/本月/累计佣金统计
- **团队管理**: 直推/间推成员列表和详情
- **趋势分析**: 每日佣金趋势图数据
- **排行榜**: 今日/本周/本月/总榜佣金排名
- **佣金预估**: 购买商品前预估可获得的佣金
...
GET /distribution/statistics/team?level=1          # 团队列表
GET /distribution/statistics/daily?days=7          # 每日统计
GET /distribution/statistics/ranking?type=1&limit=10  # 排行榜
GET /distribution/statistics/estimate?productId=1&quantity=1  # 预估佣金
```

#### 3. iOS内购支付验证
已实现完整的iOS内购验证流程:
- **自动环境切换**: 智能识别沙盒和生产环境收据
- **详细错误处理**: 提供21种状态码的详细错误说明
- **交易信息提取**: 自动提取产品ID、交易ID等关键信息
- **安全验证**: 与Apple服务器实时验证收据真实性

**使用方式**: 在支付订单时传入iOS收据即可自动验证
