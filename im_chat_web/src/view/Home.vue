<template>
	<div class="home-page" @click="closeAllBox">
		<div class="app-container" :class="{ fullscreen: isFullScreen }">
			<div class="navi-bar">
				<div class="navi-bar-box" :class="{ fullscreen: configStore.fullScreen }">
					<div class="top">
						<div class="avater">
...
export default {
	...
	methods: {
		...
		handlePrivateMessage(msg) {
			...
			// 消息插入
			if (this.$msgType.isNormal(msg.type) || this.$msgType.isTip(msg.type) || this.$msgType.isAction(msg.type) || this.$msgType.isRedPackage(msg.type)) {
		...
		insertPrivateMessage(friend, msg) {
			...
			if (!friend.isDnd && !this.chatStore.isLoading() &&
				!msg.selfSend && (this.$msgType.isNormal(msg.type) || this.$msgType.isRedPackage(msg.type))
		...
		onSwtichFullScreen() {
			...
			this.configStore.setFullScreen(this.isFullScreen);
		},
		onExit() {
			this.unloadStore();
			this.$wsApi.close(3000);
