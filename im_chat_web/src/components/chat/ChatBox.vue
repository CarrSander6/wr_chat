		...
		<el-container>
			...
			<el-main style="padding: 0;">
				<el-container>
						...
						<el-main class="im-chat-main" id="chatScrollBox" @scroll="onScroll">
							<div class="im-chat-box">
								<div v-for="(msgInfo, idx) in showMessages" :key="showMinIdx + idx">
									...
									@grabRedPackage="onGrabRedPackage">
								</chat-message-item>
								</div>
							</div>
						</el-main>
					...
					<el-aside class="chat-group-side-box" :class="{ fullscreen: configStore.fullScreen }" width="265px"
						v-if="showSide">
...
export default {
	...
	methods: {
		...
		onGrabRedPackage(redPackageId) {
			// 显示确认对话框
			this.$confirm('确认打开红包?', '红包', {
				confirmButtonText: '打开红包',
				cancelButtonText: '取消',
				type: 'warning'
			}).then(() => {
				// 调用红包领取API
				this.$http({
					...
					if (result.success) {
						this.$message.success(`恭喜您领取到${result.amount}元红包!`);
						// 更新用户余额
						if (this.userStore && this.userStore.userInfo) {
							this.userStore.userInfo.balance = result.balance;
						}
					} else {
						this.$message.warning(result.message || '红包领取失败');
					}
				}).catch((error) => {
					console.error('红包领取失败:', error);
					this.$message.error('红包领取失败，请稍后重试');
				});
			}).catch(() => {
				// 用户取消操作
			});
		...
		onReceiveRedPackage(redPackageId) {
			...
			this.$http({
				...
				if (result.success) {
					this.$message.success(`恭喜您领取到${result.amount}元红包!`);
					// 触发余额更新事件
					this.$emit('updateBalance', result.balance);
				} else {
					this.$message.warning(result.message || '红包领取失败');
				}
			}).catch((error) => {
		...
		onUpdateBalance(balance) {
			// 更新用户余额
			if (this.userStore && this.userStore.userInfo) {
				this.userStore.userInfo.balance = balance;
			}
		},
		...
		readedMessage() {
			if (this.chat.unreadCount == 0 && !this.chat.atMe) {
				return;
			}
		...
		messageSize: {
			handler(newSize, oldSize) {
				if (newSize > oldSize) {
					...
					if (lastMessage && (this.$msgType.isNormal(lastMessage.type) || this.$msgType.isRedPackage(lastMessage.type))) {
						if (this.isInBottom) {
							this.scrollToBottom();
						} else {
							this.newMessageSize++;
