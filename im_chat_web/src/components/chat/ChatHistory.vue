...
export default {
	...
	computed: {
		...
		allMessage() {
			return this.chat.messages.filter(m => this.$msgType.isNormal(m.type) || this.$msgType.isRedPackage(m.type));
		},
		imageMessage() {
		...
		messages() {
				return this[this.tabName + 'Message'].filter(m => {
					...
					// 只有文字、文件和红包支持检索
					if (this.tabName != 'all' && this.tabName != 'text' && this.tabName != 'file' && this.tabName != 'redpackage') {
						return true;
					}
					...
					} else if (this.$enums.MESSAGE_TYPE.RED_PACKAGE == m.type) {
						// 红包消息支持按祝福语搜索
						try {
							const redPackageData = JSON.parse(m.content);
							return redPackageData.blessing && redPackageData.blessing.toLowerCase().includes(this.searchText.toLowerCase());
						} catch (e) {
							return false;
						}
					}
