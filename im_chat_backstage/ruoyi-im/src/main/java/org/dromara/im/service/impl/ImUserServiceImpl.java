...
public class ImUserServiceImpl implements IImUserService {
    ...
    /*@Override
    public boolean modifyPasswordBackend(ModifyPasswordForm modifyPasswordForm) {
        ImUser update = new ImUser();
        update.setUserId(modifyPasswordForm.getUserId());
        if (ObjectUtil.equal(modifyPasswordForm.getType(), 1)) {
            update.setPassword(BCrypt.hashpw(modifyPasswordForm.getPassword()));
        } else if (ObjectUtil.equal(modifyPasswordForm.getType(), 2)) {
            update.setTradePassword(BCrypt.hashpw(modifyPasswordForm.getPassword()));
        }
    ...
    public Boolean insertByBo(ImUserBo bo) {
        ...
        add.setPassword(passwordEncoder.encode(bo.getPassword()));
    ...
    public Boolean updateByBo(ImUserBo bo) {
        ...
        //update.setPassword(passwordEncoder.encode(bo.getPassword()));
    ...
    @Override
    public Boolean modifyPassword(ImUserBo bo) {
        ImUser update = MapstructUtils.convert(bo, ImUser.class);
        update.setPassword(passwordEncoder.encode(bo.getPassword()));
        boolean flag = baseMapper.updateById(update) > 0;
        if (flag) {
    ...
    private LambdaQueryWrapper<ImUser> buildQueryWrapper(ImUserBo bo) {
        ...
        wrapper.like(StringUtils.isNotBlank(bo.getWalletAddress()), ImUser::getWalletAddress, bo.getWalletAddress());
