<template>
	<view class="page user-info">
		...
		<view v-else>
			...
			<bar-group v-if="isFriend">
				<arrow-bar title="设置备注" @tap="onRemark()">
					<text class="arrow-text">{{friendInfo.showNickName}}</text>
				</arrow-bar>
			...
			<bar-group>
				<btn-bar v-show="isFriend && !fromGroup" type="primary" title="发送消息" @tap="onSendMessage()"></btn-bar>
				<btn-bar v-show="isFriend && !fromGroup" type="danger" title="删除好友" @tap="onDelFriend()"></btn-bar>
				<btn-bar v-show="!isFriend && !isWaitingApprove && !fromGroup" type="primary" title="加为好友"
					@tap="onAddFriend()"></btn-bar>
				<btn-bar v-show="!isFriend && isWaitingApprove && !fromGroup" title="等待对方验证"></btn-bar>
			</bar-group>
		</view>
...
export default {
	...
	methods: {
		...
		onComplaint() {
			uni.navigateTo({
				url: `/pages/complaint/complaint-initiate?type=1&id=${this.userInfo.id}
	...
	onLoad(options) {
		...
		// 判断是否来自群聊
		this.fromGroup = options.fromGroup === 'true';
		// 查询用户信息
