<template>
	<view class="tab-page group">
		<nav-bar title="群聊" :add="canCreateGroup" search @add="onCreateNewGroup" @search="onSearch()"></nav-bar>
...
export default {
	...
	computed: {
		...
		canCreateGroup() {
			// 只有VIP用户(类型3)和管理员(类型4)才能创建群聊
			const userType = this.userStore.userInfo.type;
