  ...
  <view class="chat-message-item" :class="active ? 'active' : ''">
    ...
    <view class="message-normal" v-else-if="isNormal" :class="{ 'message-mine': msgInfo.selfSend }">
      <head-image class="avatar" @longpress.prevent="$emit('longPressHead')" :id="msgInfo.sendId" :url="headImage"
        :name="showName" size="small" :clickable="false"></head-image>
      <view class="content">
        <view v-if="msgInfo.groupId && !msgInfo.selfSend" class="top">
          <view class="name">{{ showName }}</view>
          <uni-tag v-if="isGroupOwner(msgInfo.sendId)" text="群主" size="small" type="error" :inverted="true"></uni-tag>
        ...
        <view class="bottom">
          <view v-if="msgInfo.type == $enums.MESSAGE_TYPE.TEXT">
            <long-press-menu :items="menuItems" @select="onSelectMenu">
              ...
              <rich-text v-if="$emo.containEmoji(msgInfo.content)" class="message-text" :nodes="nodesText"></rich-text>
          ...
          <long-press-menu v-if="msgInfo.type == $enums.MESSAGE_TYPE.USER_CARD" :items="menuItems"
            ...
            <view class="message-card" @click="onShowUserInfo">
              <view class="card-body">
                <head-image :url="contentData.headImage" :name="contentData.nickName" radius="10%" :clickable="false"></head-image>
          ...
          <long-press-menu v-if="msgInfo.type == $enums.MESSAGE_TYPE.GROUP_CARD" :items="menuItems"
            ...
            <view class="message-card" @click="onShowGroupInfo">
              <view class="card-body">
                <head-image :url="contentData.headImage" :name="contentData.groupName" radius="10%" :clickable="false"></head-image>
                <view class="name">{{ contentData.groupName }}</view>
              </view>
              <view class="card-tip">群名片</view>
            </view>
          </long-press-menu>
          <view v-if="msgInfo.type == $enums.MESSAGE_TYPE.RED_PACKAGE">
            <red-package-item :msgInfo="msgInfo" @claim-success="$emit('claimSuccess', $event)"
              @claim-already-taken="$emit('claimAlreadyTaken', $event)"></red-package-item>
          </view>
...
<script>
import RedPackageItem from '../red-package-item/red-package-item.vue';
...
export default {
  ...
  computed: {
    ...
    isNormal () {
      ...
      return this.$msgType.isNormal(type) || this.$msgType.isAction(type) || this.$msgType.isRedPackage(type)
    },
    isOwner () {
